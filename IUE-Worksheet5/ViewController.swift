...
class ViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    @IBOutlet var panGestureR: UIPanGestureRecognizer!
    ...
    private func animateTransitionIfNeeded (state: State, duration:TimeInterval) {
        if runningAnimations.isEmpty {
            let frameAnimator = UIViewPropertyAnimator(duration: duration,dampingRatio: 1) {
                ...
                switch state {
                    case .open:
                        self.bottomView.frame.origin.y = self.view.frame.height - self.bottomViewOpenedHeight
                        self.bottomView.layer.cornerRadius = 30
                    case .closed:
                        self.bottomView.frame.origin.y = self.view.frame.height -
                            (self.view.window?.safeAreaInsets.bottom ?? 0) - self.bottomViewClosedHeight
    ...
    @IBAction private func handleBottomPan(recognizer: UIPanGestureRecognizer) {
        ...
        switch recognizer.state {
            ...
            case .changed:
                ...
                fractionComplete = bottomViewIsOpened ? fractionComplete : -fractionComplete
